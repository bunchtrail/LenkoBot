...
Последняя локальная проверка web-search slice: `304 passed`, `compileall`, Ruff,
`uv lock --check` и `git diff --check`. Owner-scoped menu проверено через
реальный Bot API `setMyCommands`/`getMyCommands`; callback round-trip требует
отдельного запуска long polling.

Текущая база ещё не умеет:

- сохранять transcript turns и summaries;
- выполнять задачи, scheduler, reminders, outbox и retry;
- показывать reminder keyboards и localized dates/quiet-hours feedback (ждёт Phase 3);
...
Persona picker и command menu не используют task/reminder state и могут быть
выполнены раньше Phase 3 как отдельный ранний UX-срез. Confirmation и reminder
callbacks по-прежнему ждут Phase 3.
...
Следующая рабочая точка: Phase 2.5 — voice и personality, затем Phase 3.
Новая stateful vertical по-прежнему обязана регистрировать purge hook в
`ResetCoordinator` и проходить общие failure semantics до начала реализации.
