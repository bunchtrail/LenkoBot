...
| AI | `gpt-5.6-terra` через подписочный OAuth Codex и официальный SDK `openai-codex`; платный API-key fallback не добавляется. Прежнее ограничение «только xAI» отменено владельцем 26 июля 2026, см. [codex-oauth-2026-07.md](../analysis/codex-oauth-2026-07.md) |
...
- owner-scoped Telegram command menu и inline persona picker по `display_name`;
- одно редактируемое `status -> final` сообщение с fallback на новое сообщение,
  Telegram typing action и safe splitting длинных ответов;
- durable one-time confirmations для `/new` и `/forget` с owner-bound action
  hash, expiry и atomic replay protection;
- inline pagination для `/memories` и per-record delete buttons для `/forget`.
- config-gated web search: xAI client-side `web_search` function call, keyless
  DDGS backend, optional Tavily через `TAVILY_API_KEY`, Telegram search-progress
  edit и отдельный HTML-блок реально использованных источников.

Последняя интегрированная локальная проверка после Phase 3: `365 passed`,
`compileall`, Ruff, `uv lock --check`, `git diff --check` и package build.
Owner-scoped menu ранее проверено через реальный Bot API для семи pre-reminder
команд; текущие одиннадцать команд, callback round-trip и due reminder delivery
требуют отдельного live запуска long polling.

Текущая база ещё не умеет:

- читать произвольные URL и строить knowledge base; search slice уже доступен
  через optional `[web_search]`;
...
| `persona` / `persona_version` | immutable identity/voice history; Phase 2.5 adds the minimal runtime foundation, Phase 6 owns the canonical service | Persona service |
...
**Scope:** `Confirmed`: владелец выбрал отдельную voice & personality
вертикаль с character identity, role-structured prompt, extraction после
доставки, persona voice pack и обновлением identity без рестарта.

**Порядок:** `Assumed`: вертикаль выполняется сразу после Phase 2 и до Phase 3;
Phase 3 от неё не зависит. ID `2.5` выбран для сохранения ссылок на уже
зафиксированное evidence без перенумерации фаз.

**Цель:** бот отвечает в подтверждённом владельцем характере без роботских
шаблонов, а владелец может менять личность без рестарта.

**Текущий статус (19 июля 2026):** runtime foundation реализует immutable
persona versions, version-pinned lanes, voice TOML, structured prompt input,
post-delivery extraction recovery и owner-only reload. Live system-role smoke,
retention policy и deterministic template lint закрыты локальными evidence;
owner acceptance corpus остаётся обязательным открытым gate.

Работы:

- добавить минимальную additive migration для `persona_version`: immutable
  `identity_prompt`, `display_name`, voice pack, явный config
  `identity_version`, content hash и timestamps; `persona_session` получает
  ссылку на конкретную version без смены существующих identifiers; версия, на
  которую ссылается lane, сохраняется после restart;
- считать Phase 2.5 runtime foundation для `persona_version`; Phase 6 расширяет
  её до canonical `PersonaService`, web CRUD, publish/archive и полноценной
  version history;
- написать полный character identity для Lenko в `config.toml`: характер, тон,
  анти-шаблоны (запрещённые клише) и примеры реплик; редактирование config
  обязано явно увеличивать `identity_version`, а reload не генерирует скрытую
  версию, которая потеряется после restart;
- заменить один плоский string prompt на role-structured input: identity
  передаётся отдельным system message, transcript turns — как user/assistant
  messages, memory/relationship/summary — как отдельные untrusted data sections;
  контракт массива сообщений подтверждён
  [xai-responses.md](../analysis/xai-responses.md), а допустимость system role
  проверяется одним non-sensitive live smoke до включения; неподдержанный role
  set не подменяется молчаливым fallback и блокирует принятие этой prompt-схемы
  до отдельного решения;
- зафиксировать точный additive TOML contract для каждой persona:
  ...
  Каждая voice collection bounded, optional и содержит только allowlisted
  placeholders; renderer сохраняет смысл ответа и не принимает произвольные
  provider/model text как status или error template;
- добавить owner-only `/persona reload`: перечитать и провалидировать TOML тем
  же startup validation, принять только новую identity с большим явным
  `identity_version`, атомарно сохранить `persona_version` и переключить
  runtime catalog; старые lanes разрешают свою сохранённую version, новые turns
  используют новую; invalid или non-monotonic config не меняет active catalog;
- перенести per-turn memory extraction после успешной доставки final response:
  durable extraction run создаётся до доставки, после неё передаётся узкому
  `ExtractionCoordinator`, который сериализует processing по persona lane;
  in-process task не является единственным источником durability;
- при process crash pending run остаётся в SQLite; startup recovery и входящий
  turn перед context build выполняют bounded drain pending runs той же lane;
  следующий context build не стартует до завершения этого drain, а reset epoch
  блокирует stale processing;
- `Confirmed`: retention/reaping policy для `persona_version` сохраняет версии,
  на которые ссылаются active lanes или durable summaries. Foreign key запрещает
  удалить version, пока существует lane reference; startup/open migration не
  выполняет automatic time-based или orphan reaping. Explicit owner reset
  удаляет owned sessions/lanes и каскадные данные, но не удаляет persona history.
  Будущая очистка orphan versions требует отдельного решения и regression test;
- собрать deterministic template lint: запрещённые роботские клише отсутствуют
  в voice pack и command/status/error templates, а identity prompt содержит
  anti-template guidance;
- собрать owner acceptance corpus из живых сценариев: greeting, короткий ответ,
  disagreement/correction, применение preference, uncertainty/error и
  эмоциональный контекст; для каждого сценария фиксируются config hash, model,
  дата и owner approval; критерии включают отсутствие canned opener, стабильный
  голос, естественную вариативность и применение сохранённой preference;
- streaming и `status -> final` редактирование в эту фазу не входят и остаются
  в Phase 4; Phase 4 обязан переиспользовать voice pack для редактируемых
  status/final сообщений.

Gate:

- migration сохраняет существующие conversation/persona-session IDs и позволяет
  после restart восстановить точный identity prompt и voice pack старой active
  lane;
- banned-phrase lint зелёный для всех status/error/command templates и voice
  variants; ни один user-facing текст не раскрывает secrets, raw provider errors
  или reasoning; lint не считается доказательством качества model output;
- transcript передаётся role-structured messages; memory, relationship и summary
  остаются явно помеченными untrusted data и не становятся identity/instruction;
  prompt-injection regression corpus проходит;
- prompt budget остаётся детерминированным и ограниченным при role-structured
  assembly;
- response port наблюдает FINAL до claim/start extraction processing; crash после
  доставки оставляет durable pending run, startup/next-turn recovery её
  обрабатывает, failure оставляет run retryable, а reset не допускает stale
  activation;
- owner-only `/persona reload` применяется без рестарта; unauthorized reload
  отклоняется до config read; invalid или non-monotonic config оставляет active
  catalog без изменений; старые lanes сохраняют exact version;
- persona-version retention regression подтверждает FK protection для referenced
  lanes и отсутствие automatic orphan reaping после restart;
- owner acceptance corpus пройден владельцем по зафиксированным критериям, а не
  только по отсутствию отдельных фраз.

### Phase 3. Tasks и durable reminders

**Зависит от:** Phase 2 и общей migration/reset boundary.

`Confirmed` 21 июля 2026: recurrence использует stdlib `zoneinfo`, pinned
`tzdata` и собственный bounded daily/weekly/monthly iterator; rationale и DST
corpus сохранены в [phase3-recurrence-dst-2026-07.md](../analysis/phase3-recurrence-dst-2026-07.md).
Telegram Bot API не даёт idempotency key: gate трактуется как отсутствие
duplicate logical run/outbox; crash после externally accepted send до local
persistence остаётся честно задокументированным at-least-once transport risk.
...
- crash между claim, execution и delivery не создаёт duplicate logical run;
...
Persona picker и command menu не используют task/reminder state и были выполнены
раньше Phase 3 как отдельный UX-срез. Confirmation и reminder callbacks теперь
подключены к durable Phase 3 state; live callback round-trip остаётся manual evidence.

Работы:

- зарегистрировать owner-scoped command menu через `set_my_commands` до polling;
...
- заменить отдельные status/final messages на одно редактируемое
  `status -> final` сообщение, сохранив typed event boundary и переиспользовав
  persona voice pack Phase 2.5 для текстов этих сообщений;
...
Текущий search slice закрывает provider research, model-decided function call,
controlled degraded response и Telegram citations. Он не считается завершением
Phase 5: URL reader, snapshots, FTS, SSRF/DNS policy и общий security audit
boundary остаются следующими вертикалями этой фазы.
...
- **Personas:** расширить minimal `persona_version` foundation Phase 2.5 до
  canonical `PersonaService`, migration config-seeded personas в versioned
  catalog без смены существующих session identifiers, затем create/edit, version,
  publish, archive и active lane;
...
| Personas/voice | Immutable identity version, reload, TOML validation, role prompt, voice template lint, owner acceptance corpus |
...
1. `Closed` 27 июля 2026: выбран провайдер model inference — `gpt-5.6-terra`
...
Следующая рабочая точка: опубликовать локально проверенную Phase 3/OSS vertical,
запустить hosted CI и live reminder smoke; owner acceptance Phase 2.5 остаётся
открытым. Новая stateful vertical по-прежнему обязана регистрировать purge hook
в `ResetCoordinator` и проходить общие failure semantics до начала реализации.
Phase 2.5 не считается завершённой, пока identity history, extraction recovery
и owner acceptance corpus не имеют observable evidence.
